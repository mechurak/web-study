...
> **핵심 질문**
> - XSS와 CSRF(Cross-Site Request Forgery)는 각각 무엇을 속이는 공격인가? (브라우저를 속이기 vs 서버를 속이기)
...
- [ ] JWT(JSON Web Token)를 localStorage에 두면 안 되는 이유 (XSS와의 조합)
...
### 우리 스택의 책임 지도
- [ ] Supabase 직접 경로: RLS(Row Level Security)가 최후 방어선 (02-supabase 03장) + anon key는 공개 전제
