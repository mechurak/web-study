...
| **컴포넌트** | RTL + Vitest | 컴포넌트 하나를 가짜 DOM(jsdom)에서 렌더링해 상호작용 | 빠름 | 중간 — 조립은 검증 못 함 |
...
- GitHub Actions에서 PR마다 실행: lint + typecheck + Vitest(빠름, 항상) + Playwright(핵심 시나리오만). 04-deployment 04장의 CI 파이프라인에 편입되고, 브랜치 보호로 "깨지면 머지 불가"가 된다.
