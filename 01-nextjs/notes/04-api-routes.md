...
1. **구조가 없다.** Route Handler는 "URL당 함수 하나"가 전부다. 검증, 권한 검사, 비즈니스 로직, DB 접근이 전부 한 함수에 쌓인다. 엔드포인트가 30개가 되면 — 공통 로직은 어디에? 트랜잭션 경계는? 팀원마다 다르게 짠다. NestJS는 모듈/DI(Dependency Injection, 의존성 주입)/레이어(컨트롤러-서비스-리포지토리)로 이 답을 **프레임워크가 강제**한다.
...
- [ ] 메모 CRUD(Create/Read/Update/Delete)를 Route Handler로 구현: `GET/POST /api/memos`, `GET/DELETE /api/memos/[id]` (저장은 03장과 같은 메모리 배열 공유). curl 또는 브라우저로 직접 호출해 상태 코드(200/201/400/404)까지 확인
  → **의미**: "화면 없이 URL로만 존재하는 응답"을 만들어본다. 여기서 짠 CRUD가 03-nestjs 02장에서 컨트롤러/서비스/DTO(Data Transfer Object)로 재구현되는 비교 대상이 된다 — 같은 기능을 두 방식으로 짜봐야 구조의 가치가 보인다.
