...
- [ ] **북마크/링크 공유 서비스** — CRUD(Create, Read, Update, Delete) + 태그 + 공개/비공개(RLS, Row Level Security) + OG(Open Graph) 미리보기(백엔드 크롤링)
- [ ] **팀 TODO / 칸반** — 실시간 동기화 + 멤버 초대(인가 로직) + 활동 로그
- [ ] **간단 블로그 플랫폼** — SSG(Static Site Generation)/ISR(Incremental Static Regeneration) 활용 + 이미지 업로드 + SEO(Search Engine Optimization)
...
| 2. 설계 | `docs/design.md` — 화면 흐름, DB 스키마(ERD, Entity-Relationship Diagram), API 명세, Next↔Nest 역할 분담 | ⬜ |
| 3. DB 구축 | 마이그레이션 + RLS 정책 | ⬜ |
| 4. 백엔드 구현 | `backend/` — NestJS | ⬜ |
| 5. 프론트 구현 | `frontend/` — Next.js | ⬜ |
| 6. 배포 | Vercel + Nest 호스팅 + 도메인 + CI(Continuous Integration) | ⬜ |
